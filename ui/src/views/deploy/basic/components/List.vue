...
import { getList, deleteBasic } from '@/api/basic'
...
export default {
  ...
  methods: {
    ...
    handleDeploy(task_id) {
      createDeploy({ task_id: task_id, deploy_type: 'bc' }).then(() => {}).catch(error => {
        this.$notify({
          title: '错误',
      ...
      window.location.href = '/#/deploy/tasks/?task_id=' + task_id + '&deploy_type=bc'
    },
