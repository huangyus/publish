<template>
  <div>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false" title="新建工作流上线单">
      ...
      <el-row type="flex" justify="center">
        <el-form ref="dataForm" :model="form" :rules="rules">
          <el-form-item :label-width="formLabelWidth" label="名称" prop="name" style="margin-top: 20px;">
            <el-input v-model="form.name" placeholder="请输入上线单名称" style="width: 400px" />
          </el-form-item>
          ...
          <div v-for="(item, index) in form.steps" :key="item.idx">
            ...
            <el-form-item v-if="form.steps[index].idc.length !== 0 && active===item.idx" :label-width="formLabelWidth" label="服务器分组">
              <el-col :span="12">
                <el-select v-model="form.steps[index].servers" placeholder="请选择服务器分组" clearable filterable multiple style="width: 400px" @visible-change="groupfetchData(form.steps[index].idc)">
            ...
            <el-form-item v-if="form.steps[index].updownline && active===item.idx" :label-width="formLabelWidth" :rules="form.steps[index].updownline ? { required: true, message: 'Nginx网关必填', trigger: 'change' } : []" :prop="'steps.' + index + '.gateway'" label="Nginx网关">
              <el-col :span="12">
                <el-select v-model="form.steps[index].gateway" placeholder="请选择Nginx网关" clearable filterable multiple style="width: 400px" @visible-change="apigatewayfetchData(form.steps[index].idc)">
...
import { createWorkFlow, deleteWorkFlow, filterVersion } from '@/api/workflow'
  ...
  methods: {
    ...
    resetForm() {
      this.form = {
        ...
        confirm: false,
        steps: [
          { idx: 1, project: null, modules: null, version: null, layout: null, updownline: false, serial: false, gateway: null, servers: null, env: null, idc: [], desc: null },
          { idx: 2, project: null, modules: null, version: null, layout: null, updownline: false, serial: false, gateway: null, servers: null, env: null, idc: [], desc: null },
          { idx: 3, project: null, modules: null, version: null, layout: null, updownline: false, serial: false, gateway: null, servers: null, env: null, idc: [], desc: null }
        ]
      }
      this.radio = 1
    },
