  ...
  <div>
    <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" border fit highlight-current-row>
      ...
      <el-table-column label="部署类型" align="center" width="120">
        <template slot-scope="scope">
          <el-tooltip v-if="scope.row.cron" class="item" placement="top-start">
            <div slot="content">任务计划开始于: {{ scope.row.periodic }}</div>
            <el-tag>{{ scope.row.task_type }}</el-tag>
          </el-tooltip>
          <el-tag v-else>{{ scope.row.task_type }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="240">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['run_deploy']" v-if="scope.row.task_type==='simple' && scope.row.rb_status" size="mini" type="danger" @click="handleRediret(scope.row, 'rollback')">回滚</el-button>
...
import { getList, deleteBusiness } from '@/api/business'
