<template>
  <div>
    <el-row :gutter="20" style="margin-bottom: 15px;">
      <el-col :span="1.5">
        <el-button v-permission="['add_basic']" type="primary" icon="el-icon-edit" @click="handleCreate">新建模版</el-button>
      </el-col>
      <el-col :span="6">
...
// import { getList, createBasic } from '@/api/basic'
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          console.log(data)
          bctemplate.createTemplate(data).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          console.log(data)
          bctemplate.updateTemplate(data).then(response => {
            console.log(response)
            this.dialogFormVisible = false
            bus.$emit('List')
            this.$notify({
